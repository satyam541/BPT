...
class NetworkonlinePayment
{
    ...
    public $payModeType 	 	 = ''; // (CC)-Credit Card, (DC)-Debit Card, (DD)-Direct Debit, (PAYPAL)-PayPal, (NB)-Net Banking . Note: This field is required for â€˜Merchant Hostedâ€™ type of integration.
    public $transactionType 	 = ''; // 01 for SALE 02 for AUTHORIZATION
    public $currency 	 	 	 = 'SGD'; // AED  Used to specify the currency for the transaction. Currently, only AED is accepted for all transactions (as per ISO Currency Code).
    ...
    */
    ...
    public $UDF1   = '12365';  // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
    public $UDF2   = '';  // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
    public $UDF3   = '';  // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
    public $UDF4   = '';  // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction. 
    public $UDF5   = '';  // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
    public $UDF6   = '';  // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
    public $UDF7   = '';  // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
    public $UDF8   = '';  // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
    public $UDF9   = '';  // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
    public $UDF10  = '';  // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.	

    // Define the Field Existence Indicator for the Other Details Data Block , These are optional values	
    public $custID			       = '';  // The ID used to identify your customer when their profile was created. This field has to be passed by you only if you have opted for the â€˜Tokenization/Stored Cardâ€™ feature.
                                                                              //The field is â€˜alphanumericâ€™; special characters are not allowed.
    ...
    public $productInfo            = '12365';  // This field is used to send details about the product and related information.						
    public $isUserLoggedIn         = '';  // Indicates whether a customer has signed in to your website, or has done a Guest checkout Values: â€˜Yâ€™ â€“ customer has signed in
                                                                              //â€˜Nâ€™ â€“ customer has done a Guest checkout	 						
    public $itemTotal              = '';  // This is a comma-separated list of the sale value of all the items in the order.
                                                                              //Example: If the order has two items of value 500 and 1000, send the value as â€˜500.00, 1000.00â€™. Up to 2 decimal places are allowed in the value.						
    ...
    public function __construct(){
            ...
            //$this->beforeEncryptionString ="1110110||11111111|000000284|37.00|https://www.souqtajmeel.com/networkonlinepayment/nolpay/result/|https://www.souqtajmeel.com/networkonlinepayment/nolpay/result/|INTERNET|CC|01|AED||1110100111000|Abrar|Ahmed|Buheira Corniche|Sharjah|AE|abrar_1979@hotmail.com|0555126110||111010010001|Abrar|Ahmed|Buheira Corniche|Sharjah|AE|0555126110||1000000000|83.110.215.116||1111101|2|Web|English EYELASH ADHESIVE FOR INDIVIDUAL LASHES - CLEAR|Y|15.2400|FALSE ";


            //join('||',array_filter($this->DataBlocksArray));
    ...
    public static function create($totalAmount = 0, $currencyCode = NULL, $options = array())
    {
            
        $timestamp      = $options['timestamp']?? null;
        $gatewayOrderId = $options['gatewayOrderId']?? NULL;
        $responseUrl    = $options['responseUrl']?? NULL;
        $billingData    = $options['billingData']?? array();

            if($currencyCode == "USD")
            {// testing environment
                $merchantId = '201905191000002';
                $encryption_key = 'RNzquAEZiRH1UxCTTtXAlCd0aSotEjtnf2JvvGSkAXU=';
                $live = false;
            }
            else
            {
                // default
                $merchantId = '201907301000003';
                $encryption_key = 'Qh9r/3UDxjBZWlZsX4PRg4JKsxDp5bMJN3npu3i8K0s=';
                $live = true;
            }
            ...
            {
                $billingData = array();
                $billingData["billToFirstName"]     = null;
                $billingData["billToLastName"]      = null;
                $billingData["billToStreet1"]       = null;
                $billingData["billToStreet2"]       = null;
                $billingData["billToCity"]          = null;
                $billingData["billToState"]         = null;
                $billingData["billtoPostalCode"]    = null;
                $billingData["billToCountry"]       = null;
                $billingData["billToEmail"]         = null;
            }
        $networkOnlineArray = array('Network_Online_setting' => array(
											
                //'merchantKey'    => "O3fmRMGe5jFU2K64fZv0yDtEc0FqkGMVHYfMg1jcxrA=", 	  	   	    	// The Network Online Test
                //'merchantId'     => '201905191000001', // staging																
                'merchantKey'    => $encryption_key, 	  	   	    	// The Network LIVE
                'merchantId'     => $merchantId, // live																
                ...
                'url'	         => $live              // Set to false if you are using testing environment , set to true if you are using live environment
                ),
                ...
                'Field_Existence_Indicator_Transaction' => array(
                                        'merchantOrderNumber'  => $gatewayOrderId, 
                                        'amount'  		=> $totalAmount,
                                        //'amount'  		=> '4012.00',
                                        ...
                                        'payModeType'          => '',
                                        'transactionType'      => '01',
                                        'currency'             => empty($currencyCode)? "GBP":$currencyCode, //currency code goes here $currencyCode after setting currency on payment gateway
                                        //'currency'             => 'USD'
                ),
                'Field_Existence_Indicator_Billing' => $billingData,
                ...
                'Field_Existence_Indicator_Merchant'  => array(
                                                        'UDF1'   => $currencyCode,     // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
                                                        'UDF2'   => $timestamp,        // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
                                                        'UDF3'   => $gatewayOrderId,   // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
                                                        'UDF4'   => 'abc',             // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
                                                        'UDF5'   => 'abc',             // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
                                                        'UDF6'   => 'abc',             // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
                                                        'UDF7'   => 'abc',             // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
                                                        'UDF8'   => 'abc',             // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
                                                        'UDF9'   => 'abc',             // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.
                                                        'UDF10'  => 'abc'              // This is a â€˜user-defined fieldâ€™ that can be used to send additional information about the transaction.								
                ),
                ...
                return new self($networkOnlineArray);
    }
